...
- **Tool config volume** (parameter **`tool_config_volume_mode`**): **Dedicated** (default) creates a **second PVC per workspace** (`tool_config_disk_size`, default 5 GiB) at **`/mnt/coder-tool-config`** (ReadWriteOnce; name `coder-<workspace-id>-tool-config`). **Shared pool** creates **one ReadWriteMany PVC per Coder workspace owner** named **`coder-<owner-id>-tool-config`** (same size parameter; no admin PVC or subpaths). Prefer **one** dkai-agent shared_pool workspace per owner — a second may hit **PVC AlreadyExists** until the claim is imported into that workspace’s Terraform state. On each boot the startup script **`mkdir`**s layout there, **merges** legacy **`/home/coder`** paths for kube, gh, coder, and rancher into the PVC then removes them; **`/root/.config/glab-cli`** is only **`ln -sfn`** to the PVC (no legacy merge). **`ln -sfn`** also **`/root/.kube`**, **`/root/.config/gh`**, **`/root/.config/coder`**, **`/root/.rancher`** → that volume (tool CLIs are expected to run as **root** in this pod). **`kubectl` / `gh` / `glab` / `coder` / `rancher`** as user **`coder`** do not use this PVC unless you point them at **`$HOME=/root`** or equivalent.

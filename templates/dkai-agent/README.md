...
- **Tool config volume** (parameter **`tool_config_volume_mode`**): **Dedicated** (default) creates a **second PVC per workspace** (`tool_config_disk_size`, default 5 GiB) at **`/mnt/coder-tool-config`**. **Shared pool** uses one **admin-created** **ReadWriteMany** PVC in the namespace (`tool_shared_pvc_name`, default `coder-dkai-agent-tool-shared`) and a per-username subpath so tool config is shared across dkai-agent workspaces. (Auto-creating that PVC with `kubernetes_manifest` is not compatible with typical Coder provisioner RBAC.) On each boot the startup script **`mkdir`**s layout there, **copies** any pre-existing real dirs from `~` into it once, then **`ln -sfn`** points **`~/.kube`**, **`~/.config/gh`**, **`~/.config/glab`**, and **`~/.rancher`** at that volume so credentials survive restarts independently of the home disk.

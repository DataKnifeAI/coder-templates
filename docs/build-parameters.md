...
| DKAI Agent (`dkai-agent`) | Same CPU/memory/home disk as DKAI Arch, plus **tool_config_volume_mode** (**dedicated** default vs **shared_pool**): dedicated uses **tool_config_disk_size** (1–50 GiB) for a per-workspace second PVC at `/mnt/coder-tool-config`; shared pool requires a pre-created RWX namespace PVC (**tool_shared_pvc_name**, default `coder-dkai-agent-tool-shared`) with a per-username subpath for shared tool config; **cursor_worker_idle_timeout** (30–86400 s, default 600) for `start-cursor-worker`; optional **cursor_api_key** (masked; sets `CURSOR_API_KEY`). |

...
| DKAI Agent (`dkai-agent`) | Same CPU/memory/home disk as DKAI Arch, plus **tool_config_disk_size** (1–50 GiB, default 5, monotonic ↑) for a second PVC at `/mnt/coder-tool-config` (kubectl/gh/glab/rancher configs via symlinks); **cursor_worker_idle_timeout** (30–86400 s, default 600) for `start-cursor-worker`; optional **cursor_api_key** (masked; sets `CURSOR_API_KEY`). |
